...
struct AddItemSheet: View {
        ...
        VStack(alignment: .leading, spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                LabeledField("Category") {
                    TextField("Rifle, Optic, Magazine...", text: $metadataDraft.category)
                }

                LabeledField("Manufacturer") {
                    TextField("Radian Weapons", text: $metadataDraft.manufacturer)
                }

                HStack(spacing: 14) {
                    LabeledField("MSRP") {
                        TextField("Optional", text: $metadataDraft.msrp)
                    }
                    LabeledField("Availability") {
                        TextField("In Stock / Out of Stock / Backorder", text: $metadataDraft.availabilityStatus)
                    }
                }

                LabeledField("Purpose") {
                    TextField("Home Defense, Hunting...", text: $metadataDraft.purpose)
                }

                LabeledField("Official Product URL") {
                    TextField("Optional", text: $metadataDraft.officialProductURL)
                }

                LabeledField("Best Retailer URL") {
                    TextField("Optional", text: $metadataDraft.bestRetailerURL)
                }

                LabeledField("Primary Image URL") {
                    TextField("Optional", text: $metadataDraft.primaryImageURL)
                }

                LabeledField("Specifications") {
                    TextEditor(text: $metadataDraft.specifications)
                        .frame(minHeight: 72)
                        .padding(6)
                ...
                HStack(spacing: 14) {
                    LabeledField("Weight") {
                        TextField("Optional", text: $metadataDraft.weight)
                    }
                    LabeledField("Caliber") {
                        TextField("Optional", text: $metadataDraft.caliber)
                    }
                }

                LabeledField("Compatibility") {
                    TextField("AR-15, M-LOK...", text: $metadataDraft.compatibility)
                }

                LabeledField("Date Retrieved") {
                    TextField("YYYY-MM-DD", text: $metadataDraft.dateRetrieved)
                }

                LabeledField("Description") {
                    TextEditor(text: $metadataDraft.itemDescription)
                        .frame(minHeight: 72)
                        .padding(6)
                        .background(.background, in: RoundedRectangle(cornerRadius: 8))
                }
                ...
                LabeledField("Notes") {
                    TextEditor(text: $metadataDraft.notes)
                        .frame(minHeight: 72)
                        .padding(6)
                        .background(.background, in: RoundedRectangle(cornerRadius: 8))
                }
    ...
    private func submit() {
        ...
        if metadataDraft.hasInvalidMSRP {
            errorMessage = "MSRP must be a valid number"
            return
        }
        if metadataDraft.hasInvalidDateRetrieved {
