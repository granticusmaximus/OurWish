...
function hasCollaborativePairUniqueConstraint(database: Database): boolean {
  ...
  for (const row of indexList[0].values) {
    ...
    const indexedColumns = indexInfo[0].values
      .map((indexRow: unknown[]) => String(indexRow[2]))
      .sort();
...
export async function initializeDatabase() {
  ...
  try {
    db.run(`ALTER TABLE wish_list_items ADD COLUMN quantity INTEGER NOT NULL DEFAULT 1`);
  } catch {
    // Column already exists, ignore error
  }
  ...
  try {
    db.run(`ALTER TABLE collaborative_items ADD COLUMN quantity INTEGER NOT NULL DEFAULT 1`);
  } catch {
    // Column already exists, ignore error
  }
  ...
  db.run(
    `INSERT OR IGNORE INTO users (first_name, last_name, display_name, email, password)
     VALUES (?, ?, ?, ?, ?)`,
    ['Grant', 'Watson', 'Grant', 'grant@gwsapp.net', 'Wats#0529']
  );
    ...
    for (const row of users[0].values) {
      ...
      if (!existingLists.length || !existingLists[0].values || existingLists[0].values.length === 0) {
        ...
        if (listResult.length > 0 && listResult[0].values && listResult[0].values.length > 0) {
          ...
          db.run(
            'UPDATE wish_list_items SET list_id = ? WHERE user_id = ? AND list_id IS NULL',
            [listId, userId]
          );
        }
      }
...
export function runUpdate(sql: string, params: any[] = []): Promise<{ id: number; changes: number }> {
  return new Promise((resolve, reject) => {
    try {
      if (!db) {
        throw new Error('Database not initialized');
