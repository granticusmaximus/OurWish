...
router.post('/register', async (req: AuthRequest, res: Response) => {
  ...
  try {
    const userCount = await runQuery<CountRow>('SELECT COUNT(*) as count FROM users');
    if (userCount[0].count >= 2) {
      return res.status(403).json({ error: 'Maximum users (2) already registered' });
    }

    const result = await runUpdate(
      `INSERT INTO users (first_name, last_name, display_name, email, password)
       VALUES (?, ?, ?, ?, ?)`,
      [firstName, lastName, firstName, email, password]
    );

    // Seed a default personal wish list for the new user.
    await runUpdate(
      'INSERT INTO wish_lists (user_id, name) VALUES (?, ?)',
      [result.id, 'My Wish List']
    );
...
router.get('/me', async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const lists = await runQuery<WishListRow>(
      'SELECT name FROM wish_lists WHERE user_id = ? ORDER BY created_at ASC LIMIT 1',
      [req.session.userId]
    );
...
// Check user count
router.get('/user-count', async (req: AuthRequest, res: Response) => {
  try {
    const result = await runQuery<CountRow>('SELECT COUNT(*) as count FROM users');
