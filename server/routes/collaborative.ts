...
async function scrapeProductInfo(url: string): Promise<{ name: string; price: number } | null> {
  try {
...
router.get('/partners', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const partners = await runQuery<PartnerUserRow>(
...
router.post('/create', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    // Find partner user
    const partners = await runQuery<PartnerUserRow>(
      'SELECT id, email, display_name FROM users WHERE LOWER(email) = LOWER(?) AND id != ?',
      [normalizedPartnerEmail, req.userId]
    );
    ...
    const result = await runUpdate(
      `INSERT INTO collaborative_lists (name, user1_id, user2_id)
       VALUES (?, ?, ?)`,
      [normalizedListName, req.userId, partnerId]
    );
...
router.get('/lists', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const lists = await runQuery<any>(
...
router.get('/lists/:listId/items', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const lists = await runQuery<any>(
      `SELECT * FROM collaborative_lists 
       WHERE id = ? AND (user1_id = ? OR user2_id = ?)`,
      [listId, req.userId, req.userId]
    );
    ...
    const items = await runQuery<any>(
      `SELECT * FROM collaborative_items 
...
router.post('/lists/:listId/items', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const result = await runUpdate(
      `INSERT INTO collaborative_items (list_id, product_name, price, quantity, url)
       VALUES (?, ?, ?, ?, ?)`,
      [listId, productName, price, normalizedQuantity, url === undefined ? null : url]
    );
...
router.put('/lists/:listId/items/:itemId', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    await runUpdate(
      ...
      [product_name, price, normalizedQuantity, url === undefined ? null : url, itemId, listId]
    );
...
router.put('/lists/:listId/items/:itemId/purchase', requireAuth, async (req: AuthRequest, res: Response) => {
  const { listId, itemId } = req.params;
...
router.delete('/lists/:listId/items/:itemId', requireAuth, async (req: AuthRequest, res: Response) => {
  const { listId, itemId } = req.params;
  ...
  try {
    ...
    await runUpdate(
      `DELETE FROM collaborative_items 
       WHERE id = ? AND list_id = ?`,
      [itemId, listId]
    );
...
router.delete('/lists/:listId', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    await runUpdate(
      `DELETE FROM collaborative_lists WHERE id = ?`,
      [listId]
    );
